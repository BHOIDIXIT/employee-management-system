...
const Sidebar = () => {
        ...
        <>
            ...
            <div className="flex-1 px-3 space-y-0.5 overflow-y-auto">
                {navItems.map((item) => {
                    ...
                    return (
                        <Link key={item.href} to={item.href} className="group relative flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/5">
                            {isActive &&
                                <div className='absolute left-0 top-1/2 
                                                -translate-y-1/2 w-[3px] h-5 
    ...
    return (
        <>
          ...
          to-slate-950 text-white shrink border-r border-white/4'>
                {sidebarContent}
            </aside>

            {/* Sidebar - mobile */}
            <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900  via-slate-900 to-slate-950 text-white
    z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-tracking-x-full'}`}></aside>
        </>
    )
}
